...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers including negatives") {
    assert(sum(List(1, 2, 0, -3, -5, 7)) === 2)
  }
  ...
  test("max of a few numbers including negatives") {
    assert(max(List(3, 7, 2, -8, -3, 10, -12)) === 10)
  }
