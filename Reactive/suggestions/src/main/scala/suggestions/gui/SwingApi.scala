...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
     ...
     */
    def textValues: Observable[String] = Observable.create(createTextFieldObserver)

    def createTextFieldObserver(o: Observer[String]): Subscription = {

      field.subscribe {
        case ValueChanged(textField) => o.onNext(textField.text)
        case _ => ()
      }
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.create(createButtonObserver)

    def createButtonObserver(o: Observer[Button]): Subscription = {
