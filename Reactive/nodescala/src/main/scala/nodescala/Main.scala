...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val timeOut: Future[String] = {
      Future(Await.result(Future("Server timeout!"), Duration(20, TimeUnit.SECONDS)))
    }
    ...
    val terminationRequested: Future[String] = {
      ...
      userInterrupted onComplete { x =>p.tryComplete(_) }
      timeOut onComplete { x =>p.tryComplete(_) }
