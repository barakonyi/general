...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      val p = Promise[T]()
      p.completeWith(Future.successful(value)).future
    }
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs match {
        case Nil => Future(Nil)
        case (f::ft) => f.flatMap { t => all(ft).flatMap { ts => Future(t::ts) } }
      }
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs.map { future => {future onComplete {p.tryComplete(_)}} }
      p.future
    }
    ...
    def delay(t: Duration): Future[Unit] = async {
      ...
      Await.result(never, Duration.Zero)
    }
