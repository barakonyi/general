...
class BinaryTreeSet extends Actor {
  ...
  // Actor 1 - normal: adds to the pending queue or set underGC and became the runNext state.
  // Actor 2 - runNext: 
  //                - underGC == true -> copy root and garbageCollecting
  //                - pendingQueue.isEpty -> back to normal
  //                - else: running
  // Actor 3 - running: adding operations to pending queue, setting up underGC state, but changing actor back to runNext only when Operation is finished.
  // Actor 4 - garbageCollecting: adding operations to pending queue, setting up underGC state, but changing actor back to runNext only when GC is finished.
  // optional
    ...
    if (underGC) {
      ...
      if (pendingQueue.isEmpty)
        ...
        val nextOp = pendingQueue.head match {
          ...
          case r: Remove => r.copy(requester = context.self)
          case c: Contains => c.copy(requester = context.self)
        }
        root ! nextOp
        running
      }
  ...
  def garbageCollecting(newRoot: ActorRef): Receive = LoggingReceive {
    ...
    case GC => {
      underGC = true // In GC state, we receive another GC message -> run GC again!
    }
    case _ =>
  }
